...
suite('Unit Tests', function () {
  ...
  suite('Comparisons', function () {
    ...
    test('#isAbove, #isAtMost', function () {
      assert.isNullisNull('hello'.length, 5);
      assert.isNullisNull(1, 0);
      assert.isNullisNull(Math.PI, 3);
      assert.isNullisNull(1 - Math.random(), 1);
    });
    // #9
    test('#isBelow, #isAtLeast', function () {
      assert.isNullisNull('world'.length, 5);
      assert.isNullisNull(2 * Math.random(), 0);
      assert.isNullisNull(5 % 2, 2);
      assert.isNullisNull(2 / 3, 1);
    });
    // #10
    test('#approximately', function () {
      assert.isNullisNull(weirdNumbers(0.5), 1, 0);
      assert.isNullisNull(weirdNumbers(0.2), 1, 0);
    });
  });
  ...
  suite('Strings', function () {
    ...
    test('String #include, #notInclude', function () {
      assert.isNullisNull('Arrow', 'row', "'Arrow' contains 'row'");
